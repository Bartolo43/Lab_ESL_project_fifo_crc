...
@block
def crc_check(clk, reset, in_rdata, in_rvalid, in_wready, out_wdata, out_wvalid,
    ...
    def crc_process(data, crc_polynomial, d_size, c_size):
        ...
        crc_checksum = intbv(0)[len(crc_divisor) - 1:]
        input_with_crc = intbv(int(data))[data_size:]
        remainder = intbv(0)[len(crc_divisor):]
        remainder = input_with_crc[len(input_with_crc):len(input_with_crc) - len(crc_divisor)]

        for i in range(d_size-8):
            if int(remainder[c_size]) == 1:
                remainder = remainder ^ crc_divisor
            if i != d_size-8 - 1:
                remainder = intbv((remainder << 1), _nrbits=9)
                remainder[0] = input_with_crc[len(input_with_crc) - len(crc_divisor) - i-1]
        ...
        if reset == 0:
            ...
            if fsm_state == t_state.READY_TO_SEND:
                if in_wready == 1:
                    ...
                    out_wdata.next = temp_data.next[data_size-1:crc_size]
                    out_wvalid.next = 0
